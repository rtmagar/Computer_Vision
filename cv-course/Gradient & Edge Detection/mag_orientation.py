...
# load the image and convert it to grayscale, and display the original image
...
# compute the gradient magnitutde and direction respectively
mag = np.sqrt((gX ** 2) + (gY ** 2))
orientation = np.arctan2(gY, gX) * (180/ np.pi) % 180
orientation = np.arctan2(319, 23) * (180/ np.pi) % 180
print(orientation)

# find all pixels that are within upper and low angle boundaries
